...
Electronic lab notebooks (ELNs), including LabArchives [2], are widely used for experimental record management, but their contents are typically accessed through platform-specific interfaces that are not designed for direct use by AI assistants. =lab_archives_mcp= is a Python package that provides a Model Context Protocol (MCP) [1] server for LabArchives and a configurable vector-search backend for semantic retrieval over notebook content. The server exposes notebooks, pages, and entries as typed MCP tools, supports project-scoped state across sessions, and provides optional write and upload operations that record provenance metadata for computational outputs. By keeping records in the institutional ELN while mediating access through explicit API calls and configurable indexing backends, the software supports reusable ELN integration patterns for AI-assisted research workflows. The software is available on GitHub and Zenodo under the MIT License.
...
#+CAPTION: High-level architecture of the MCP integration. The AI assistant communicates with =lab_archives_mcp=, which mediates access to the LabArchives API while isolating authentication, API-client, vector-search, state-management, and write/provenance responsibilities inside separable Python modules.
[[file:docs/figures/architecture.png]]
...
3. *MCP server* (=mcp_server.py=): Exposes notebook operations as MCP tools using the FastMCP framework [4]. In =v0.4.0=, =mcp_server.py= registers 18 MCP tools by default: 16 discovery, reading, search/index, project-state, provenance, and onboarding tools, plus the write-capable =write_notebook_entry= and =upload_to_labarchives= tools. Setting =LABARCHIVES_ENABLE_UPLOAD=false= disables those two write-capable tools, leaving a 16-tool read/search/state surface.
...
6. *Upload and provenance layer* (=models/upload.py=): Provides a high-level write and upload API for appending rich text entries (=write_notebook_entry=) and archiving computational artefacts directly into LabArchives pages (=upload_to_labarchives=), capturing rich provenance metadata including Git commit SHA, repository URL, execution timestamp, Python version, and dependency versions.
...
*Institutional deployment:* Research data management teams can deploy the MCP server as part of an institutional AI assistant, exposing ELN content under existing authentication and logging policies. The project documents multiple deployment options for the vector backend, including local Parquet persistence and self-hosted vector stores, allowing institutions to keep embeddings and metadata on-premise when required. When managed embedding providers or vector stores such as Pinecone and Qdrant are enabled, notebook-derived text, embeddings, metadata, or both may be stored on third-party infrastructure; these backends should therefore be treated as explicit deployment choices requiring local data-governance review.
