...
block = f"""[mcp_servers.labarchives]
command = "conda"
args = [
  "run",
  "--no-capture-output",
...
pattern = re.compile(r"(?ms)^\\[mcp_servers\\.labarchives\\].*?(?=^\\[|\\Z)")
if pattern.search(data):
    data = pattern.sub(block.rstrip() + "\n\n", data)
else:
...
echo "Codex setup complete. Restart Codex to load the MCP server config."
