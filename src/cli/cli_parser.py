...
def parse_and_dispatch_cli(argv: list[str] = None) -> int:
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"CLI command '{args.command}' invoked",
                extra={
        ...
        try:
            ...
            if handler_func is None:
                ...
                logger.error(
                    "No handler function found for command",
                    extra={
                        "operation": "parse_and_dispatch_cli",
                        "command": args.command,
                ...
                print(
                    f"Error: No handler found for command '{args.command}'",
                    file=sys.stderr,
                )
                return 1
            ...
            logger.info(
                f"Dispatching to handler for command '{args.command}'",
                extra={
                    "operation": "parse_and_dispatch_cli",
                    "command": args.command,
            ...
            logger.info(
                f"Command '{args.command}' completed",
                extra={
                    "operation": "parse_and_dispatch_cli",
                    "command": args.command,
                    "exit_code": exit_code,
        ...
        if logger:
            logger.info(
                "CLI operation cancelled by user",
                extra={
                    "operation": "parse_and_dispatch_cli",
