...
def reload_configuration() -> ServerConfiguration:
    ...
    try:
        ...
        reloaded_config = load_configuration(
            cli_args=_LAST_CLI_ARGS, config_file_path=_LAST_CONFIG_FILE_PATH
        )
...
def _merge_configuration_dicts(
    base_config: Dict[str, Any], overlay_config: Dict[str, Any], source_name: str
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if (
            merged_config.get('verbose') is True
            and merged_config.get('quiet') is True
            and (('verbose' in overlay_config) != ('quiet' in overlay_config))
        ):  # Different sources
        ...
        overlay_scopes = [
            ...
            if opt in overlay_config and overlay_config[opt] is not None
        ]
