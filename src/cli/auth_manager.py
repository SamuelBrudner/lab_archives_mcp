...
class AuthenticationManager:
    ...
    def authenticate(self) -> 'AuthenticationSession':
        ...
        try:
            ...
            if self.config.username:
                ...
                self.logger.info(
                    "Using user token authentication method",
                    extra={
                        'component': 'AuthenticationManager',
                        'operation': 'authenticate',
    ...
    def get_session(self) -> 'AuthenticationSession':
        ...
        self.logger.debug(
            "Accessing authentication session",
            extra={
                'component': 'AuthenticationManager',
        ...
        if self.session is None or not self.session.is_valid():
            ...
            if self.session is None:
                self.logger.info(
                    "No authentication session found, triggering authentication",
                    extra={
                        'component': 'AuthenticationManager',
                        'operation': 'get_session',
                        'reason': 'no_session',
                ...
                self.logger.info(
                    "Current session is invalid, triggering re-authentication",
                    extra={
                        'component': 'AuthenticationManager',
                        'operation': 'get_session',
                        'reason': 'session_invalid',
                        'session_expired': self.session.expires_at is not None
                        and is_token_expired(self.session.expires_at),
                    },
                )
        ...
        # Check for proactive session refresh if session is valid but approaching expiration
            ...
            self.logger.info(
                "Session approaching expiration, triggering proactive refresh",
                extra={
                    'component': 'AuthenticationManager',
                    'operation': 'get_session',
            ...
            try:
                self.session = self.authenticate()
                self.logger.info(
                    "Proactive session refresh completed successfully",
                    extra={
                        'component': 'AuthenticationManager',
                        'operation': 'get_session',
                        'new_expires_at': (
                ...
                self.logger.warning(
                    "Proactive session refresh failed, continuing with current session",
                    extra={
                        'component': 'AuthenticationManager',
                        'operation': 'get_session',
                        'error': str(e),
        ...
        self.logger.debug(
            "Valid authentication session accessed",
            extra={
                'component': 'AuthenticationManager',
                'operation': 'get_session',
                'user_id': self.session.user_id,
    ...
    def refresh_session(self) -> 'AuthenticationSession':
        ...
        self.logger.info(
            "Explicit session refresh requested",
            extra={
                'component': 'AuthenticationManager',
        ...
        if self.session is not None:
            ...
            self.logger.debug(
                "Invalidating current session for refresh",
                extra={
                    'component': 'AuthenticationManager',
                    'operation': 'refresh_session',
    ...
    def invalidate_session(self) -> None:
        ...
        if self.session is not None:
            ...
            self.logger.debug(
                "Session invalidation requested but no session exists",
                extra={
                    'component': 'AuthenticationManager',
                    'operation': 'invalidate_session',
                    'had_session': False,
        ...
        self.logger.info(
            "Authentication session invalidated successfully",
            extra={
                'component': 'AuthenticationManager',
                'operation': 'invalidate_session',
    ...
    def __str__(self) -> str:
        ...
        """
        return (
            f"AuthenticationManager(api_base_url={self.config.api_base_url}, "
