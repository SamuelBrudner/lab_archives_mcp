        ...
        if is_sensitive_parameter(key):
            ...
            if isinstance(value, list):
                ...
                sanitized[key] = [
                    (
                        _REDACTION_MARKER
                        if isinstance(item, str) and is_sensitive_parameter(key)
                        else item
                    )
                    for item in value
                ]
...
def sanitize_url_params(url: str) -> str:
    ...
    try:
        ...
        sanitized_url = f"{parsed.scheme}://" f"{parsed.netloc}" f"{parsed.path}"
...
def validate_sanitization_complete(text: str) -> bool:
    ...
    has_redaction = any(
        re.search(pattern, text, re.IGNORECASE) for pattern in redaction_patterns
    )
