    ...
    try:
        ...
        if not isinstance(resource_info, dict) or 'type' not in resource_info:
            logger.error(
                ...
                extra={
                    "resource_info": str(resource_info)[
                        :100
        ...
        if scope_config.notebook_id:
            return _validate_notebook_id_scope(
                resource_info, scope_config.notebook_id, logger
            )
        elif scope_config.notebook_name:
            return _validate_notebook_name_scope(
                resource_info, scope_config.notebook_name, additional_context, logger
            )
        elif scope_config.folder_path:
            return _validate_folder_path_scope(
                resource_info, scope_config.folder_path, additional_context, logger
            )
        else:
        ...
        if scope_config.notebook_id:
            if entry_notebook_id == scope_config.notebook_id:
                logger.debug(
                    ...
                    extra={
                        **audit_context,
                        "validation_result": "allowed_notebook_id_match",
                    },
                )
                return True
            else:
    ...
    try:
        ...
        try:
            ...
            raise FolderScopeViolationError(
                ...
                code=400,
                context={"folder_scope": folder_scope, "parse_error": str(e)},
            )
...
def is_resource_in_scope(
    resource_info: Dict[str, Any], scope_config: ScopeConfig
) -> bool:
    """
...
def _has_any_scope_configured(scope_config: ScopeConfig) -> bool:
    ...
    return bool(
        scope_config.notebook_id
        or scope_config.notebook_name
        or scope_config.folder_path
    )
...
def _validate_notebook_id_scope(
    resource_info: Dict[str, Any], notebook_id: str, logger
) -> bool:
    ...
    if resource_type == 'notebook':
        ...
        if resource_notebook_id == notebook_id:
            logger.debug(
                f"Notebook {resource_notebook_id} matches scope - access allowed"
            )
            return True
        else:
            logger.warning(
                f"Notebook {resource_notebook_id} outside scope [{notebook_id}]"
            )
        ...
        if resource_notebook_id == notebook_id:
            logger.debug(
                f"{resource_type.title()} belongs to notebook {resource_notebook_id} within scope"
            )
            return True
        else:
            logger.warning(
                f"{resource_type.title()} belongs to notebook {resource_notebook_id} outside scope [{notebook_id}]"
            )
...
def _validate_notebook_name_scope(
    ...
    if additional_context and 'notebook_name' in additional_context:
        ...
        if actual_notebook_name == notebook_name:
            logger.debug(
                f"Notebook name '{actual_notebook_name}' matches scope - access allowed"
            )
            return True
        else:
            logger.warning(
                f"Notebook name '{actual_notebook_name}' outside scope ['{notebook_name}']"
            )
            raise NotebookScopeViolationError(
...
def _validate_notebook_folder_access(
    ...
    if not resource_metadata or 'notebook_folders' not in resource_metadata:
        logger.warning(
            ...
            extra={
                **audit_context,
                "validation_result": "denied_insufficient_metadata",
            },
        )
        raise FolderScopeViolationError(
    ...
    if scope_folder.is_root:
        ...
        has_pages_in_scope = len(notebook_folders) > 0 or resource_metadata.get(
        ...
        for folder_path_str in notebook_folders:
            try:
                ...
                if (
                    scope_folder.is_parent_of(folder_path)
                    or scope_folder.components == folder_path.components
                ):
...
def _validate_page_folder_access(
    ...
    page_folder_path = (
        resource_metadata.get('page_folder_path') if resource_metadata else None
    )
    ...
    if page_folder_path in ('', None):
        if scope_folder.is_root:
            logger.debug(
                ...
                extra={
                    **audit_context,
                    "validation_result": "allowed_root_page_root_scope",
                },
            )
            return True
        else:
            logger.debug(
                ...
                extra={
                    **audit_context,
                    "validation_result": "denied_root_page_non_root_scope",
                },
            )
            return False
    ...
    if (
        scope_folder.is_root
        or scope_folder.is_parent_of(page_folder)
        or scope_folder.components == page_folder.components
    ):
        logger.debug(
...
def _validate_entry_folder_access(
    ...
    page_metadata = {'page_folder_path': parent_page_folder_path}
