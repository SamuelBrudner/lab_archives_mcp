...
class User(BaseModel):
    ...
    uid: constr(min_length=1, max_length=255) = Field(
        description="Unique user identifier from LabArchives", example="user_987654"
    )

    name: constr(min_length=1, max_length=255) = Field(
        description="User's full name", example="Dr. Sarah Johnson"
    )

    email: EmailStr = Field(
        description="User's email address", example="sarah.johnson@university.edu"
    )
...
class PageMetadata(BaseModel):
    ...
    id: constr(min_length=1, max_length=255) = Field(
        description="Unique page identifier from LabArchives API", example="page_789012"
    )

    notebook_id: constr(min_length=1, max_length=255) = Field(
        description="Parent notebook identifier", example="nb_123456"
    )
    ...
    created_date: datetime = Field(
        description="Timestamp when page was created", example="2024-01-16T09:15:00Z"
    )
...
class EntryContent(BaseModel):
    ...
    page_id: constr(min_length=1, max_length=255) = Field(
        description="Parent page identifier", example="page_789012"
    )
    ...
    created_date: datetime = Field(
        description="Timestamp when entry was created", example="2024-01-16T10:30:00Z"
    )
    ...
    author: constr(min_length=1, max_length=255) = Field(
        description="Username of the entry author", example="researcher@university.edu"
    )

    version: conint(ge=1) = Field(
        description="Entry version number for tracking changes", example=3
    )
...
class UserContext(BaseModel):
    ...
    user: User = Field(description="User identity and profile information")

    status: constr(min_length=1, max_length=100) = Field(
        description="Authentication status", example="success"
    )
