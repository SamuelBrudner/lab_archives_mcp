...
from requests.adapters import HTTPAdapter  # requests>=2.31.0 - For connection pooling and retries
from requests.packages.urllib3.util.retry import Retry  # requests>=2.31.0 - For retry strategy
...
class LabArchivesAPIClient:
    ...
    def __init__(self, access_key_id: str, access_password: str, 
                 username: Optional[str] = None, api_base_url: str = DEFAULT_API_BASE_URL):
        """
        ...
        self.logger.info("LabArchives API client initialized", extra={
            "api_base_url": api_base_url,
            "access_key_id": access_key_id[:8] + "..." if len(access_key_id) > 8 else "[short]",
            "username": username,
    ...
    def _build_authenticated_params(self, additional_params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
    ...
    def _make_request(self, method: str, endpoint: str, params: Optional[Dict[str, Any]] = None,
        ...
        try:
            ...
            if response.status_code == 403:
                self.logger.error("Permission denied", extra={
                    "status_code": response.status_code,
                    "endpoint": endpoint,
                    "method": method
                })
            ...
            if not response.ok:
                ...
                self.logger.error(f"API request failed: {error_message}", extra={
                    "status_code": response.status_code,
                    "endpoint": endpoint,
                    "method": method,
            ...
            self.logger.info(f"API request successful: {method} {endpoint}", extra={
                "status_code": response.status_code,
                "endpoint": endpoint,
                "method": method,
            ...
            self.logger.error(f"Request timeout: {method} {endpoint}", extra={
                "endpoint": endpoint,
                "method": method,
            ...
            self.logger.error(f"Connection error: {method} {endpoint}", extra={
                "endpoint": endpoint,
                "method": method,
                "error": str(e)
            })
            ...
            self.logger.error(f"Unexpected error: {method} {endpoint}", extra={
                "endpoint": endpoint,
                "method": method,
                "error": str(e),
    ...
    def handle_rate_limit(self, retry_count: int) -> None:
        ...
        if retry_count >= MAX_RETRIES:
            self.logger.error("Rate limit exceeded - max retries reached", extra={
                "retry_count": retry_count,
        ...
        self.logger.warning(f"Rate limit encountered - retrying in {delay} seconds", extra={
            "retry_count": retry_count,
            "delay_seconds": delay
        })
    ...
    def authenticate(self) -> UserContextResponse:
        ...
        try:
            ...
            self.logger.error("Authentication failed with unexpected error", extra={
                "error": str(e),
                "error_type": type(e).__name__
            })
            raise APIAuthenticationError(
    ...
    def list_notebooks(self) -> NotebookListResponse:
        ...
        self.logger.info("Listing notebooks for user", extra={
            "user_id": self.uid,
        ...
        try:
            ...
            self.logger.error("Unexpected error listing notebooks", extra={
                "error": str(e),
                "error_type": type(e).__name__,
                "user_id": self.uid
            })
    ...
    def list_pages(self, notebook_id: str) -> PageListResponse:
        ...
        except (APIAuthenticationError, APIPermissionError, APIError, APIResponseParseError):
            # Re-raise known API errors
            raise
        except Exception as e:
            self.logger.error("Unexpected error listing pages", extra={
                "error": str(e),
                "error_type": type(e).__name__,
                "notebook_id": notebook_id,
                "user_id": self.uid
    ...
    def list_entries(self, page_id: str) -> EntryListResponse:
        ...
        try:
            ...
            self.logger.error("Unexpected error listing entries", extra={
                "error": str(e),
                "error_type": type(e).__name__,
                "page_id": page_id,
                "user_id": self.uid
    ...
    def get_entry_content(self, entry_id: str) -> EntryListResponse:
        ...
        self.logger.info("Getting entry content", extra={
            "entry_id": entry_id,
            "user_id": self.uid,
        ...
        try:
            ...
            self.logger.error("Unexpected error getting entry content", extra={
                "error": str(e),
                "error_type": type(e).__name__,
                "entry_id": entry_id,
                "user_id": self.uid
