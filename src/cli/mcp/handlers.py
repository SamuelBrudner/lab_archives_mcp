...
from typing import (
    ...
    Dict,
    Any,
)  # builtin - Type hinting for handler methods and message structures

# Import internal dependencies for resource management and data models
from .resources import MCPResourceManager
from .models import MCPResourceListResponse, MCPResourceReadResponse
from .errors import (
    handle_mcp_error,
    MCP_ERROR_CODES,
    MCP_ERROR_MESSAGES,
...
from constants import DEFAULT_PROTOCOL_VERSION, MCP_SERVER_NAME, MCP_SERVER_VERSION
...
def parse_jsonrpc_message(raw_message: str) -> Dict[str, Any]:
    ...
    if message.get('jsonrpc') != '2.0':
        logger.error(
            ...
            extra={"jsonrpc_value": message.get('jsonrpc'), "expected": "2.0"},
        )
        raise MCPProtocolError(
    ...
    logger.info(
        "JSON-RPC message parsed successfully",
        extra={
            "method": message['method'],
...
def build_jsonrpc_response(result: Any = None, error: Optional[Any] = None, id: Any = None) -> str:
    ...
    if error is not None:
        ...
        logger.debug(
            "Building JSON-RPC error response",
            extra={
        ...
        logger.debug(
            "Building JSON-RPC success response",
            extra={"result_type": type(result).__name__, "request_id": id},
        )
    ...
    try:
        ...
        logger.debug(
            "JSON-RPC response built successfully",
            extra={
        ...
        logger.error(
            ...
            extra={"error": str(e), "response_data": str(response), "request_id": id},
        )
...
def route_mcp_request(request: Dict[str, Any], resource_manager: MCPResourceManager) -> str:
    ...
    logger.info(
        "Routing MCP request",
        extra={"method": method, "has_params": bool(params), "request_id": request_id},
    )
    ...
    try:
        ...
        error_response = handle_mcp_error(
            e, context={"method": method, "request_id": request_id, "params": params}
        )
...
class MCPProtocolHandler:
    ...
    def __init__(self, resource_manager: MCPResourceManager):
        ...
        self.logger.info(
            "MCPProtocolHandler initialized",
            extra={
    ...
    def handle_message(self, raw_message: str) -> str:
        ...
        self.logger.info(
            ...
            extra={"message_length": len(raw_message), "handler_instance": id(self)},
        )
        ...
        try:
            ...
            self.logger.info(
                "MCP message processed successfully",
                extra={
        ...
        try:
            ...
            while True:
                try:
                    ...
                    self.logger.error(
                        "Unexpected error in session loop",
            ...
            self.logger.info(
                "MCP protocol session terminated", extra={"handler_instance": id(self)}
            )
    ...
    def handle_initialize(request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(
            "Processing MCP initialize request",
            extra={
    ...
    def handle_resources_list(
        ...
        logger.info(
            "Processing MCP resources/list request",
        ...
        response = MCPResourceListResponse(
            resources=resources,
            metadata={"total_count": len(resources), "request_id": request_id},
        )

        # Log successful resource listing
        logger.info(
            "MCP resources/list completed successfully",
            extra={"resource_count": len(resources), "request_id": request_id},
        )
    ...
    def handle_resources_read(
        ...
        logger.info(
            "Processing MCP resources/read request",
            extra={"uri": resource_uri, "request_id": request_id},
        )
        ...
        response = MCPResourceReadResponse(
            ...
            metadata={"request_uri": resource_uri, "request_id": request_id},
        )

        # Log successful resource reading
        logger.info(
            "MCP resources/read completed successfully",
            extra={
                "uri": resource_uri,
