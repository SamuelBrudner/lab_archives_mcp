...
def parse_resource_uri(uri: str) -> Dict[str, str]:
    ...
    # Handle unsupported resource types
        ...
        raise LabArchivesMCPException(
            ...
            code=400,
            context={"uri": uri, "resource_type": resource_type, "supported_types": ["notebook", "entry"]}
        )
...
class MCPResourceManager:
    ...
    def __init__(self, api_client: LabArchivesAPIClient, scope_config: Dict[str, Any], jsonld_enabled: bool = False):
        """
        ...
        self.logger.info("MCPResourceManager initialized", extra={
    ...
    def list_resources(self) -> List[MCPResource]:
        ...
        try:
            ...
            if scope_notebook_id:
                self.logger.info("Listing pages within notebook scope", extra={
                ...
                for page in page_list_response.pages:
                    ...
                    if is_resource_in_scope(page_resource_info, self.scope_config):
                        ...
                        self.logger.warning("Page excluded due to scope restrictions", extra={
                            "page_id": page.id,
                            "notebook_id": page.notebook_id,
                            "scope_config": self.scope_config
                        })
            ...
            # Handle notebook name-based scope (resolve name to ID and list pages)
                ...
                self.logger.info("Listing pages within notebook name scope", extra={
                ...
                if target_notebook:
                    ...
                    self.logger.warning("Notebook not found with specified name", extra={
                        "notebook_name": scope_notebook_name,
                        "available_notebooks": [nb.name for nb in notebook_list_response.notebooks]
                    })
            
            # Handle no scope or unrestricted access (list all notebooks)
            else:
                self.logger.info("Listing all accessible notebooks", extra={
                    "operation": "list_all_notebooks"
                })
                ...
                for notebook in notebook_list_response.notebooks:
                    ...
                    if is_resource_in_scope(notebook_resource_info, self.scope_config):
                        ...
                        self.logger.warning("Notebook excluded due to scope restrictions", extra={
                            "notebook_id": notebook.id,
            ...
            self.logger.info("Resource listing completed successfully", extra={
                "operation": "list_resources",
            ...
            self.logger.error("LabArchives API error during resource listing", extra={
                "operation": "list_resources",
                "error": str(e),
                "error_code": e.code,
            ...
            self.logger.error("Unexpected error during resource listing", extra={
                "operation": "list_resources",
                "error": str(e),
    ...
    def read_resource(self, uri: str) -> MCPResourceContent:
        ...
        self.logger.info("Starting resource read operation", extra={
        ...
        try:
            ...
            if not is_resource_in_scope(resource_info, self.scope_config):
                self.logger.warning("Resource read denied due to scope violation", extra={
                    "uri": uri,
            ...
            if resource_type == "notebook":
                ...
                self.logger.info("Reading notebook resource", extra={
                ...
                self.logger.info("Notebook resource read successfully", extra={
                    "notebook_id": notebook_id,
            ...
            # Handle page resource reading
                ...
                self.logger.info("Reading page resource", extra={
                    "notebook_id": notebook_id,
                    "page_id": page_id,
                    "operation": "read_page"
                })
                ...
                if not target_page:
                    raise LabArchivesMCPException(
                        ...
                        code=404,
                        context={"notebook_id": notebook_id, "page_id": page_id, "uri": uri}
                    )
                ...
                self.logger.info("Page resource read successfully", extra={
                    "page_id": page_id,
            ...
            # Handle entry resource reading
                ...
                self.logger.info("Reading entry resource", extra={
                    "entry_id": entry_id,
                ...
                self.logger.info("Entry resource read successfully", extra={
                    "entry_id": entry_id,
            ...
            # Handle LabArchives API errors with detailed logging
            self.logger.error("LabArchives API error during resource reading", extra={
                "operation": "read_resource",
                "uri": uri,
                "error": str(e),
                "error_code": e.code,
            ...
            # Handle unexpected errors with comprehensive logging
            self.logger.error("Unexpected error during resource reading", extra={
                "operation": "read_resource",
                "uri": uri,
                "error": str(e),
                "error_type": type(e).__name__
