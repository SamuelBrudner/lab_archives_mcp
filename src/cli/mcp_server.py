...
def run_protocol_with_session_refresh(
    protocol_handler, auth_manager, stdin, stdout, logger, audit_logger, max_retries=3
) -> None:
    """
    ...
    while retry_count <= max_retries:
        try:
            logger.info(
                "Starting MCP protocol session loop",
                extra={
            ...
            logger.info(
                "MCP protocol session ended normally",
                extra={"operation": "protocol_session_end", "retry_count": retry_count},
            )
            return
            ...
            if is_authentication_error(e):
                ...
                if retry_count <= max_retries:
                    logger.warning(
                        f"Authentication error detected, attempting session refresh (attempt {retry_count}/{max_retries})",
                        extra={
                    ...
                    audit_logger.warning(
                        "Session refresh triggered by authentication failure",
                        extra={
                    ...
                    try:
                        ...
                        logger.info(
                            "Refreshing authentication session...",
                            extra={
                        ...
                        logger.info(
                            "Session refreshed successfully, retrying protocol operation",
                            extra={
                                "user_id": (
                                    refreshed_session.user_id
                                    if hasattr(refreshed_session, 'user_id')
                                    else "unknown"
                                ),
                        ...
                        logger.error(
                            f"Session refresh failed on attempt {retry_count}: {str(refresh_error)}",
                            extra={
                        ...
                        if retry_count < max_retries:
                            ...
                            logger.error(
                                "Maximum session refresh retries exceeded, giving up",
                                extra={
                                    "max_retries": max_retries,
                    ...
                    logger.error(
                        f"Maximum authentication retry attempts exceeded ({max_retries}), terminating session",
                        extra={
                            "max_retries": max_retries,
                    ...
                    audit_logger.error(
                        "Session terminated due to persistent authentication failures",
                        extra={
                ...
                logger.error(
                    f"Non-authentication error during protocol session: {str(e)}",
                    extra={
                        "error_type": type(e).__name__,
                        "error_message": str(e),
...
def handle_shutdown_signal(signum: int, frame) -> None:
    ...
    logger.info(
        f"Received shutdown signal {signal_name}, terminating server gracefully",
        extra={
    ...
    try:
        ...
        audit_logger.info(
            ...
            extra={
                ...
                "scope_type": (
                    ...
                    else "folder" if server_config.scope.folder_path else "all"
                ),
                "json_ld_enabled": server_config.output.json_ld_enabled,
            },
        )
        ...
        audit_logger.info(
            ...
            extra={
                ...
                "auth_method": (
                    "user_token" if server_config.authentication.username else "api_key"
                ),
            },
        )
        ...
        logger.info(
            "LabArchives MCP Server initialization completed successfully with session refresh capability",
            extra={
                "server_name": server_config.server_name,
                "server_version": server_config.server_version,
                "status": "ready",
        ...
        audit_logger.info(
            "Server initialization completed with enhanced authentication",
            extra={
                "event": "server_ready",
                "server_name": server_config.server_name,
                "server_version": server_config.server_version,
        ...
        audit_logger.info(
            ...
            extra={"event": "server_shutdown", "reason": "normal_termination"},
        )
        ...
        if logger:
            ...
            if 'audit_logger' in locals():
                audit_logger.info(
                    "Server shutdown via keyboard interrupt",
                    extra={
        ...
        if logger:
            ...
            if is_auth_error:
                logger.error(
                    "Authentication-related error during server execution",
                    extra={
                        "error_type": type(e).__name__,
                        "error_details": str(e),
                ...
                if 'audit_logger' in locals():
                    audit_logger.error(
                        "Server terminated due to authentication failure",
                        extra={
            ...
            if is_auth_error:
                print(
                    "NOTE: This appears to be an authentication-related error. Please verify your credentials and API access."
                )
