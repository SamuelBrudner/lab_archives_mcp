...
CONFIG_COMMAND_DESCRIPTION = "LabArchives MCP Server configuration management commands (show, validate, reload)."

# =============================================================================
# Main Configuration Command Functions
# =============================================================================

def add_config_subparser(main_parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    """
    ...
    try:
        ...
        reload_parser.add_argument(
            '--config-file',
            type=str,
            help='Path to configuration file to reload'
        )
        reload_parser.add_argument(
            '--verify',
            action='store_true',
...
def show_config_command(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        try:
            ...
            print(f"Warning: Failed to initialize logging: {logging_error}", file=sys.stderr)
        ...
        try:
            ...
            if logger:
                logger.error(error_msg, extra={"error_code": config_error.code, "context": config_error.context})
            if audit_logger:
                audit_logger.error(
    ...
    try:
        ...
        try:
            ...
            if validation_error.code in [2001, 2002]:  # Authentication errors
                print("  Suggestion: Check your LABARCHIVES_AKID and LABARCHIVES_SECRET environment variables", file=sys.stderr)
            elif validation_error.code in [3001, 3002, 3003, 3004]:  # Scope errors
                print("  Suggestion: Review your scope configuration (notebook_id, notebook_name, or folder_path)", file=sys.stderr)
            elif validation_error.code in [5001, 5002]:  # Logging errors
                print("  Suggestion: Check your log file path and permissions", file=sys.stderr)
            
            if logger:
                logger.error(error_msg, extra={"error_code": validation_error.code, "context": validation_error.context})
            if audit_logger:
                audit_logger.error(
...
def reload_config_command(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        try:
            ...
            if logger:
                logger.error(error_msg, extra={"error_code": reload_error.code, "context": reload_error.context})
            if audit_logger:
                audit_logger.error(
        ...
        try:
            ...
            if getattr(args, 'verify', False):
                ...
                print(f"  JSON-LD: {'Enabled' if reloaded_config.output.json_ld_enabled else 'Disabled'}")
                print(f"  Structured Output: {'Enabled' if reloaded_config.output.structured_output else 'Disabled'}")
