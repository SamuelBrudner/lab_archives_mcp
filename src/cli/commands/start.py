...
def start_command(cli_args: dict) -> int:
    ...
    try:
        ...
        logger.info("Loading and validating configuration", extra={
        ...
        try:
            ...
            logger.error("Configuration loading failed", extra={
                "operation": "start_command",
                "step": "load_configuration",
                "error": str(e),
                "error_code": e.code,
        ...
        logger.info("Starting LabArchives MCP Server", extra={
            ...
            "scope_type": (
                "notebook_id" if config.scope.notebook_id else
                "notebook_name" if config.scope.notebook_name else
                "folder_path" if config.scope.folder_path else
                "unrestricted"
            ),
        ...
        logger.info("Establishing authentication session", extra={
            "operation": "start_command",
            "step": "authentication",
            "api_base_url": config.authentication.api_base_url,
            "auth_method": "user_token" if config.authentication.username else "api_key"
        ...
        try:
            ...
            audit_logger.info("Authentication session established", extra={
            ...
            audit_logger.error("Authentication failed", extra={
        ...
        try:
            ...
            logger.error("Resource manager initialization failed", extra={
                "operation": "start_command",
                "step": "resource_manager_init",
                "error": str(e),
        ...
        logger.info("Initializing MCP protocol handler", extra={
            "operation": "start_command",
        ...
        try:
            ...
            logger.info("MCP protocol handler initialized successfully", extra={
                "operation": "start_command",
                "step": "protocol_handler_init",
            ...
            logger.error("MCP protocol handler initialization failed", extra={
                "operation": "start_command",
                "step": "protocol_handler_init",
                "error": str(e),
                "error_type": type(e).__name__
        ...
        logger.info("Starting MCP protocol session", extra={
            "operation": "start_command",
        ...
        audit_logger.info("MCP server ready for client connections", extra={
            "event": "server_ready",
            "server_name": config.server_name,
            "server_version": config.server_version,
            "user_id": session.user_id
        ...
        try:
            ...
            logger.info("MCP protocol session completed", extra={
                "operation": "start_command",
                "step": "protocol_session",
                "completion_reason": "normal"
            })
            
            audit_logger.info("MCP server session completed normally", extra={
            ...
            logger.info(f"MCP protocol session interrupted by {interruption_type}", extra={
                "operation": "start_command",
                "step": "protocol_session",
                "interruption_type": interruption_type
            })
            
            audit_logger.info("MCP server session interrupted", extra={
                "event": "session_interrupted",
                "user_id": session.user_id,
                "interruption_type": interruption_type
            })
            
            print(f"Server shutting down gracefully ({interruption_type})", file=sys.stderr)
            return 0
            
        except Exception as e:
            logger.error("Runtime error during protocol session", extra={
                "operation": "start_command",
                "step": "protocol_session",
                "error": str(e),
                "error_type": type(e).__name__
            ...
            audit_logger.error("MCP server runtime error", extra={
                "event": "runtime_error",
                "user_id": session.user_id,
                "error": str(e),
                "error_type": type(e).__name__
        ...
        logger.error("MCP server startup failed", extra={
            "operation": "start_command",
            "error": str(e),
        ...
        logger.error("Unexpected fatal error during server startup", extra={
            "operation": "start_command",
            "error": str(e),
            "error_type": type(e).__name__,
