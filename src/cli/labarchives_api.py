...
class LabArchivesAPI:
    ...
    def __init__(
        ...
        self.logger.info(
            "Initializing LabArchives API wrapper",
            extra={
                "region": self.region,
        ...
        try:
            ...
            self.logger.info(
                "LabArchives API client initialized successfully",
                extra={"region": self.region, "api_base_url": self.api_base_url},
            )

        except Exception as e:
            # Log initialization failure and re-raise as APIError
            self.logger.error(
                "Failed to initialize LabArchives API client",
                extra={
                    "error": str(e),
    ...
    def list_notebooks(self) -> NotebookListResponse:
        ...
        """
        self.logger.info(
            "Listing notebooks for authenticated user",
            extra={"region": self.region, "operation": "list_notebooks"},
        )
        ...
        try:
            ...
            self.logger.error(
                "Authentication error during notebook listing",
                extra={
                    "error": str(e),
                    "error_code": e.code,
                    "region": self.region,
            ...
            self.logger.error(
                "API error during notebook listing",
                extra={
                    "error": str(e),
                    "error_code": e.code,
                    "region": self.region,
            ...
            # Log unexpected error and re-raise as APIError
            self.logger.error(
                "Unexpected error during notebook listing",
                extra={
                    "error": str(e),
                    "error_type": type(e).__name__,
                    "region": self.region,
    ...
    def list_pages(self, notebook_id: str) -> PageListResponse:
        ...
        try:
            ...
            # Log authentication error and re-raise
            self.logger.error(
                "Authentication error during page listing",
                extra={
                    "error": str(e),
                    "error_code": e.code,
                    "notebook_id": notebook_id,
            ...
            self.logger.error(
                "Permission error during page listing",
                extra={
                    "error": str(e),
                    "error_code": e.code,
                    "notebook_id": notebook_id,
            ...
            # Log unexpected error and re-raise as APIError
            self.logger.error(
                "Unexpected error during page listing",
                extra={
                    "error": str(e),
                    "error_type": type(e).__name__,
                    "notebook_id": notebook_id,
    ...
    def list_entries(self, page_id: str) -> EntryListResponse:
        ...
        try:
            ...
            # Log authentication error and re-raise
            self.logger.error(
                "Authentication error during entry listing",
                extra={
                    "error": str(e),
                    "error_code": e.code,
                    "page_id": page_id,
            ...
            # Log permission error and re-raise
            self.logger.error(
                "Permission error during entry listing",
                extra={
                    "error": str(e),
                    "error_code": e.code,
                    "page_id": page_id,
            ...
            # Log API error and re-raise
            self.logger.error(
                "API error during entry listing",
                extra={
                    "error": str(e),
                    "error_code": e.code,
                    "page_id": page_id,
            ...
            # Log unexpected error and re-raise as APIError
            self.logger.error(
                "Unexpected error during entry listing",
                extra={
                    "error": str(e),
                    "error_type": type(e).__name__,
                    "page_id": page_id,
    ...
    def get_entry_content(self, entry_id: str) -> EntryListResponse:
        ...
        try:
            ...
            # Log authentication error and re-raise
            self.logger.error(
                "Authentication error during entry content retrieval",
                extra={
                    "error": str(e),
                    "error_code": e.code,
                    "entry_id": entry_id,
            ...
            # Log permission error and re-raise
            self.logger.error(
                "Permission error during entry content retrieval",
                extra={
                    "error": str(e),
                    "error_code": e.code,
                    "entry_id": entry_id,
            ...
            # Log API error and re-raise
            self.logger.error(
                "API error during entry content retrieval",
                extra={
                    "error": str(e),
                    "error_code": e.code,
                    "entry_id": entry_id,
            ...
            # Log unexpected error and re-raise as APIError
            self.logger.error(
                "Unexpected error during entry content retrieval",
                extra={
                    "error": str(e),
                    "error_type": type(e).__name__,
                    "entry_id": entry_id,
