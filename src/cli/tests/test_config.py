...
from unittest.mock import (
    ...
    MagicMock,
    mock_open,
)  # builtin - Mocking framework for test isolation
...
class TestLoadValidConfiguration:
    ...
    def test_load_valid_configuration_from_cli_args(self, get_valid_config):
        ...
        assert isinstance(
            result_config, ServerConfiguration
    ...
    def test_load_valid_configuration_from_config_file(self, tmp_path):
        ...
        assert (
            result_config.authentication.access_key_id == config_data['access_key_id']
        ), "Authentication access_key_id should match file value"
        assert (
            result_config.authentication.access_secret == config_data['access_secret']
        ), "Authentication access_secret should match file value"
        assert (
            result_config.scope.notebook_id == config_data['notebook_id']
        ), "Scope notebook_id should match file value"
        assert (
            result_config.logging.log_level == config_data['log_level']
        ), "Logging log_level should match file value"
        assert (
            result_config.logging.verbose == config_data['verbose']
        ), "Logging verbose should match file value"

        # Assert: Verify default values are used where not specified in file
        assert (
            result_config.output.json_ld_enabled is False
        ), "Output json_ld_enabled should use default value"
        assert (
            result_config.output.structured_output is True
    ...
    def test_load_configuration_with_environment_variables(self):
        ...
        assert (
            result_config.authentication.access_key_id == env_vars['LABARCHIVES_AKID']
        ), "Authentication access_key_id should match environment variable"
        assert (
            result_config.authentication.access_secret == env_vars['LABARCHIVES_SECRET']
        ), "Authentication access_secret should match environment variable"
        assert (
            result_config.authentication.username == env_vars['LABARCHIVES_USER']
        ), "Authentication username should match environment variable"
        assert (
            result_config.scope.notebook_id == env_vars['LABARCHIVES_NOTEBOOK_ID']
        ), "Scope notebook_id should match environment variable"
        assert (
            result_config.logging.log_level == env_vars['LABARCHIVES_LOG_LEVEL']
        ), "Logging log_level should match environment variable"
        assert (
            result_config.logging.verbose is False
    ...
    def test_load_configuration_all_sources_combined(self, tmp_path):
        ...
        with patch.dict(os.environ, env_vars):
            result_config = load_configuration(
                cli_args=cli_args, config_file_path=str(config_file)
            )
        ...
        assert (
            result_config.logging.quiet == cli_args['quiet']
        ...
        assert (
            result_config.authentication.access_secret == file_config['access_secret']
        ), "File access_secret should be used when not overridden"

        # Assert: Verify file values are used where not overridden
        assert (
            result_config.authentication.username == file_config['username']
        ), "File username should be used when not overridden by CLI or env"
        assert (
            result_config.scope.notebook_id == file_config['notebook_id']
...
class TestLoadInvalidConfigurationRaises:
    ...
    def test_load_invalid_configuration_missing_required_fields(self):
        ...
        assert "access_key_id" in str(exc_info.value) or "access_secret" in str(
            exc_info.value
        ), "Exception message should mention the missing required field"
        assert (
            exc_info.value.code is not None
        ), "Exception should have an error code for programmatic handling"
        assert (
            exc_info.value.context is not None
    ...
    def test_load_invalid_configuration_conflicting_scope_values(self):
        ...
        assert (
            "scope" in str(exc_info.value).lower()
        ), "Exception message should mention scope validation"
        assert (
            "mutual" in str(exc_info.value).lower()
    ...
    def test_load_invalid_configuration_conflicting_verbosity_flags(self):
        ...
        assert (
            "verbose" in str(exc_info.value).lower()
            and "quiet" in str(exc_info.value).lower()
    ...
    def test_load_invalid_configuration_invalid_api_base_url(self):
        ...
        assert (
            "api_base_url" in str(exc_info.value).lower()
            or "https" in str(exc_info.value).lower()
...
class TestEdgeCaseConfiguration:
    ...
    def test_edge_case_configuration_maximum_length_values(self):
        ...
        assert (
            len(result_config.authentication.access_key_id) == 256
        ), "Maximum length access_key_id should be accepted"
        assert (
            len(result_config.authentication.access_secret) == 1024
        ), "Maximum length access_secret should be accepted"
        assert (
            result_config.authentication.username == edge_case_cli_args['username']
        ), "Long username should be accepted when valid"
        assert (
            result_config.scope.notebook_id == edge_case_cli_args['notebook_id']
        ), "Notebook ID with special characters should be accepted"

        # Assert: Verify that validation doesn't reject valid edge cases
        assert (
            result_config.logging.log_level == 'DEBUG'
    ...
    def test_edge_case_configuration_special_characters_in_paths(self, tmp_path):
        ...
        config_data['log_file'] = str(
            tmp_path / "logs" / "with spaces and unicode 测试.log"
        )
        config_file.write_text(json.dumps(config_data, indent=2, ensure_ascii=False))
        ...
        assert (
            result_config.logging.log_file == config_data['log_file']
        ), "Log file path with special characters should be preserved"
        assert (
            result_config.scope.folder_path == config_data['folder_path']
    ...
    def test_edge_case_configuration_empty_optional_fields(self):
        ...
        assert (
            result_config.authentication.username is None
        ), "Username should remain None when explicitly set"
        assert (
            result_config.scope.notebook_id is None
        ...
        assert (
            result_config.logging.log_file == DEFAULT_LOG_FILE
        ), "Log file should use default when None is provided"
        assert (
            result_config.authentication.api_base_url == DEFAULT_API_BASE_URL
...
class TestReloadConfigurationIdempotency:
    ...
    def test_reload_configuration_idempotency_with_cli_args(self):
        ...
        assert isinstance(
            reloaded_config, ServerConfiguration
        ), "Reloaded configuration should be a ServerConfiguration object"

        # Assert: Verify authentication section equivalence
        assert (
            reloaded_config.authentication.access_key_id
            == initial_config.authentication.access_key_id
        ), "Reloaded authentication access_key_id should match initial"
        assert (
            reloaded_config.authentication.access_secret
            == initial_config.authentication.access_secret
        ), "Reloaded authentication access_secret should match initial"
        assert (
            reloaded_config.authentication.username
            == initial_config.authentication.username
        ), "Reloaded authentication username should match initial"
        assert (
            reloaded_config.authentication.api_base_url
            == initial_config.authentication.api_base_url
        ), "Reloaded authentication api_base_url should match initial"

        # Assert: Verify scope section equivalence
        assert (
            reloaded_config.scope.notebook_id == initial_config.scope.notebook_id
        ), "Reloaded scope notebook_id should match initial"
        assert (
            reloaded_config.scope.notebook_name == initial_config.scope.notebook_name
        ), "Reloaded scope notebook_name should match initial"
        assert (
            reloaded_config.scope.folder_path == initial_config.scope.folder_path
        ), "Reloaded scope folder_path should match initial"

        # Assert: Verify logging section equivalence
        assert (
            reloaded_config.logging.log_level == initial_config.logging.log_level
        ), "Reloaded logging log_level should match initial"
        assert (
            reloaded_config.logging.verbose == initial_config.logging.verbose
        ), "Reloaded logging verbose should match initial"

        # Assert: Verify server metadata equivalence
        assert (
            reloaded_config.server_name == initial_config.server_name
        ), "Reloaded server_name should match initial"
        assert (
            reloaded_config.server_version == initial_config.server_version
    ...
    def test_reload_configuration_idempotency_with_config_file(self, tmp_path):
        ...
        assert (
            reloaded_config.authentication.access_key_id == config_data['access_key_id']
        ), "Reloaded access_key_id should match file value"
        assert (
            reloaded_config.authentication.access_secret == config_data['access_secret']
        ), "Reloaded access_secret should match file value"
        assert (
            reloaded_config.authentication.username == config_data['username']
        ), "Reloaded username should match file value"
        assert (
            reloaded_config.logging.log_level == config_data['log_level']
        ), "Reloaded log_level should match file value"
        assert (
            reloaded_config.scope.notebook_name == config_data['notebook_name']
    ...
    def test_reload_configuration_fails_without_initial_load(self):
        ...
        """
        # Patch the module-level cache directly to guarantee isolation.
        with patch("src.cli.config._CONFIG_CACHE", None):
            # Act & Assert: The absence of a previously-loaded configuration
            # should trigger a LabArchivesMCPException from reload_configuration().
            with pytest.raises(LabArchivesMCPException) as exc_info:
                reload_configuration()

            # Assert: Verify appropriate error message and code
            assert (
                "no configuration has been loaded" in str(exc_info.value).lower()
                or "cannot reload" in str(exc_info.value).lower()
            ), "Exception should mention that no configuration was loaded previously"
            assert exc_info.value.code is not None, "Exception should have an error code"
            assert (
                exc_info.value.context is not None
            ), "Exception should have context for debugging"
    ...
    def test_reload_configuration_reflects_environment_changes(self):
        ...
        assert (
            reloaded_config.authentication.access_key_id
            == updated_env['LABARCHIVES_AKID']
        ), "Reloaded configuration should reflect updated environment variable"
        assert (
            reloaded_config.authentication.access_secret
            == updated_env['LABARCHIVES_SECRET']
        ), "Reloaded configuration should reflect updated secret"
        assert (
            reloaded_config.authentication.username == updated_env['LABARCHIVES_USER']
        ), "Reloaded configuration should reflect updated username"
        assert (
            reloaded_config.logging.log_level == updated_env['LABARCHIVES_LOG_LEVEL']
...
class TestGetConfigValueDotNotation:
    ...
    def test_get_config_value_dot_notation_missing_field_raises_exception(self):
        ...
        assert "authentication.non_existent_field" in str(
            exc_info.value
    ...
    def test_get_config_value_dot_notation_invalid_section_raises_exception(self):
        ...
        assert "non_existent_section" in str(
            exc_info.value
    ...
    def test_get_config_value_dot_notation_no_configuration_loaded_raises_exception(
        self,
    ):
        """
        ...
        """
        # Patch the module-level cache to ensure test isolation
        with patch('src.cli.config._CONFIG_CACHE', None):
...
class TestEnvironmentVariablePrecedence:
    ...
    def test_environment_variable_precedence_over_config_file(self, tmp_path):
        ...
        assert (
            result_config.logging.verbose is True
    ...
    def test_environment_variable_precedence_over_defaults(self):
        ...
        assert (
            result_config.logging.log_file == env_vars['LABARCHIVES_LOG_FILE']
    ...
    def test_environment_variable_type_conversion(self):
        ...
        env_vars = {
            ...
            # Use a compatible combination to satisfy business-rule validation:
            # json_ld_enabled=False allows structured_output to be False as well.
            'LABARCHIVES_JSON_LD_ENABLED': '0',  # Should convert to False
    ...
    def test_environment_variable_partial_override(self, tmp_path):
        ...
        assert (
            result_config.authentication.access_key_id == file_config['access_key_id']
        ...
        assert (
            result_config.logging.log_file == file_config['log_file']
        ), "File log_file should be preserved when not overridden by environment"
        assert (
            result_config.output.json_ld_enabled == file_config['json_ld_enabled']
...
class TestCLIArgumentPrecedence:
    ...
    def test_cli_argument_precedence_over_config_file(self, tmp_path):
        ...
        cli_args = {
            'access_key_id': 'AKID_CLI_OVERRIDE',
            'access_secret': 'cli_secret_override',
            # INFO level is required when json_ld_enabled is True
            'log_level': 'INFO',
            'verbose': True,
        ...
        assert (
            result_config.output.json_ld_enabled == cli_args['json_ld_enabled']
    ...
    def test_cli_argument_precedence_over_all_sources(self, tmp_path):
        ...
        assert (
            result_config.scope.folder_path == cli_args['folder_path']
    ...
    def test_cli_argument_partial_override_preserves_other_sources(self, tmp_path):
        ...
        cli_args = {
            ...
            # INFO level is required when json_ld_enabled is True
            'log_level': 'INFO',  # Override env and file
    ...
    def test_cli_argument_none_values_do_not_override(self, tmp_path):
        ...
        assert (
            result_config.logging.verbose == file_config['verbose']
        ), "None CLI argument should not override file value"
        assert (
            result_config.scope.notebook_id == file_config['notebook_id']
        ), "None CLI argument should not override file value"
