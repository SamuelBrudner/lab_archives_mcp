...
from src.cli.constants import MCP_SERVER_VERSION, DEFAULT_CLI_CONFIG_FILE
        ...
        try:
            ...
            with patch('src.cli.cli_parser.load_configuration') as mock_load_config:
                with patch('src.cli.cli_parser.setup_logging') as mock_setup_logging:
                    with patch(
                        'src.cli.commands.start.start_command'
                    ) as mock_start_command:
                        # Configure mocks
                        mock_config = Mock()
                        ...
                        exit_code = parse_and_dispatch_cli(
                            ['start', '--config-file', temp_config_path]
                        )
            ...
            with patch('src.cli.cli_parser.load_configuration') as mock_load_config:
                with patch('src.cli.cli_parser.setup_logging') as mock_setup_logging:
                    with patch(
                        ...
                        exit_code = parse_and_dispatch_cli(
                            ['start', '--access-key-id', 'AKID_FROM_CLI', '--verbose']
                        )
            ...
            with patch('src.cli.cli_parser.load_configuration') as mock_load_config:
                with patch('src.cli.cli_parser.setup_logging') as mock_setup_logging:
                    with patch(
                        ...
                        exit_code = parse_and_dispatch_cli(
                            ['start', '-k', 'AKID_FROM_CLI_SHORT', '--verbose']
                        )
                        ...
                        exit_code = parse_and_dispatch_cli(
                            ['start', '--access-key', 'AKID_FROM_CLI_LONG', '--quiet']
                        )
    ...
    def test_configuration_loading_error_handling(self):
        ...
        with patch('src.cli.cli_parser.load_configuration') as mock_load_config:
            mock_load_config.side_effect = LabArchivesMCPException(
                "Configuration loading failed", code=1001
            )
...
class TestCLIParserLoggingIntegration:
    ...
    def test_audit_logging_for_commands(self):
        ...
        with patch('src.cli.cli_parser.load_configuration') as mock_load_config:
            with patch('src.cli.cli_parser.setup_logging') as mock_setup_logging:
                with patch(
                    'src.cli.commands.authenticate.authenticate_command'
                ) as mock_auth_command:
                    # Configure mocks
                    mock_config = Mock()
                    mock_config.logging = Mock()
                    mock_load_config.return_value = mock_config
        ...
        with patch('src.cli.cli_parser.load_configuration') as mock_load_config:
            with patch('src.cli.cli_parser.setup_logging') as mock_setup_logging:
                with patch('src.cli.commands.start.start_command') as mock_start_command:
                    ...
                    mock_start_command.side_effect = Exception(
...
class TestCLIParserExceptionHandling:
    ...
    def test_labarchives_exception_handling(self):
        ...
        with patch('src.cli.cli_parser.load_configuration') as mock_load_config:
            mock_load_config.side_effect = LabArchivesMCPException(
                "Authentication failed", code=2001
            )
...
class TestCLIParserAccessKeyAliases:
    ...
    def test_access_key_flag_aliases_map_to_same_destination(self):
        ...
        args_with_long_access_key = parser.parse_args(
            ['start', '--access-key', 'TEST_KEY_LONG']
        )

        args_with_short_k = parser.parse_args(['start', '-k', 'TEST_KEY_SHORT'])

        args_with_access_key_id = parser.parse_args(
            ['start', '--access-key-id', 'TEST_KEY_ID']
        )
    ...
    def test_short_k_flag_identical_behavior_to_access_key_id(self):
        ...
        args_k = parser.parse_args(
            ['start', '-k', 'AKID1234567890ABCDEF', '--access-secret', 'test-secret']
        )
    ...
    def test_access_key_flag_identical_behavior_to_access_key_id(self):
        ...
        assert (
            args_access_key_auth.access_key_id == args_access_key_id_auth.access_key_id
        )
        assert (
            args_access_key_auth.access_secret == args_access_key_id_auth.access_secret
        )
    ...
    def test_all_commands_support_access_key_aliases(self):
        ...
        args_auth_access_key = parser.parse_args(
            ['authenticate', '--access-key', 'TEST_KEY']
        )
    ...
    def test_documented_command_behavior_with_aliases(self):
        ...
        args = parser.parse_args(
            [
                ...
                'AKID1234567890ABCDEF',
                '--access-secret',
                'test-secret',  # Note: -p is not mentioned in existing code, using --access-secret
                '--notebook-name',
                'Research',
                '--folder-path',
                'Projects/AI',
    ...
    def test_access_key_aliases_help_text_shows_preferred_format(self):
        ...
        for action in start_subparser._actions:
            if hasattr(action, 'option_strings') and any(
                opt in action.option_strings
                for opt in ['-k', '--access-key', '--access-key-id']
            ):
        ...
        assert (
            '--access-key' in access_key_action.option_strings
            or '--access-key-id' in access_key_action.option_strings
        )
...
class TestCLIParserIntegration:
    ...
    def test_complete_start_workflow(self):
        ...
        with patch.dict(
            os.environ,
            {'LABARCHIVES_AKID': 'test_key', 'LABARCHIVES_SECRET': 'test_secret'},
        ):
            with patch('src.cli.cli_parser.load_configuration') as mock_load_config:
                with patch('src.cli.cli_parser.setup_logging') as mock_setup_logging:
                    with patch(
                        ...
                        exit_code = parse_and_dispatch_cli(
                            ['start', '--verbose', '--access-key-id', 'cli_key']
                        )
