...
def _create_valid_config_instance() -> ServerConfiguration:
    ...
    scope_config = ScopeConfig(
        notebook_id="notebook_12345", notebook_name=None, folder_path=None
    )

    # Instantiate OutputConfig with standard formatting options
    output_config = OutputConfig(json_ld_enabled=False, structured_output=True)
...
def get_invalid_config() -> ServerConfiguration:
    ...
    object.__setattr__(
        auth_config, 'username', "not-an-email"
    )  # Invalid: not email format
    object.__setattr__(
        auth_config, 'api_base_url', "http://insecure.com"
    )  # Invalid: not HTTPS
    ...
    object.__setattr__(
        scope_config, 'notebook_name', "Conflicting Notebook Name"
    )  # Invalid: conflicts with notebook_id
    ...
    object.__setattr__(
        output_config, 'structured_output', False
    )  # Invalid: JSON-LD requires structured output

    # Instantiate LoggingConfig with invalid values and conflicts
    logging_config = LoggingConfig.__new__(LoggingConfig)
    object.__setattr__(
        logging_config, 'log_file', "/root/inaccessible.log"
    )  # Invalid: likely permission issues
    object.__setattr__(
    ...
    object.__setattr__(
        server_config, 'server_version', "invalid-version"
...
def get_edge_case_config() -> ServerConfiguration:
    ...
    output_config = OutputConfig(json_ld_enabled=True, structured_output=True)
