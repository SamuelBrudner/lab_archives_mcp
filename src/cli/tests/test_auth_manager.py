...
from unittest.mock import (
    ...
    MagicMock,
    call,
)  # builtin - Used to patch/mock LabArchives API client methods and simulate API responses
from datetime import (
    datetime,
    timedelta,
)  # builtin - Used for timestamp assertions and session context validation
...
from typing import (
    ...
    Any,
    Optional,
)  # builtin - Type hints for better code clarity and IDE support
...
)  # Alias for JSON spec compatibility
...
def mock_user_context():
    ...
    """
    mock_user = User(uid="user_123456", name="Test User", email="test@university.edu")

    return UserContext(
        user=mock_user, status="success", message="Authentication successful"
    )
...
async def test_authenticate_user_success(get_valid_config, mock_user_context):
    ...
    with patch('src.cli.auth_manager.LabArchivesAPI') as mock_api_class:
        ...
        assert (
            abs(actual_lifetime.total_seconds() - expected_lifetime.total_seconds()) < 5
...
@pytest.mark.asyncio
async def test_auth_manager_establish_session_and_get_context(
    get_valid_config, mock_user_context
):
    """
...
@pytest.mark.asyncio
async def test_auth_manager_get_session_triggers_authentication(
    get_valid_config, mock_user_context
):
    """
...
async def test_auth_manager_refresh_session(get_valid_config, mock_user_context):
    ...
    with patch('src.cli.auth_manager.LabArchivesAPI') as mock_api_class:
        ...
        updated_user_context = UserContext(
            user=User(
                uid="user_123456", name="Test User Updated", email="test@university.edu"
            ),
            status="success",
...
@pytest.mark.asyncio
async def test_auth_manager_session_expiration_handling(
    get_valid_config, mock_user_context
):
    """
...
@pytest.mark.asyncio
async def test_audit_logging_on_authentication(
    get_valid_config, get_invalid_config, mock_user_context
):
    """
...
def test_audit_logging_session_management(get_valid_config, mock_user_context):
    ...
    with patch('src.cli.auth_manager.LabArchivesAPI') as mock_api_class:
        ...
        with patch('src.cli.auth_manager.get_logger') as mock_logger:
            ...
            debug_calls = [
                call
                for call in logger_instance.debug.call_args_list
...
@pytest.mark.asyncio
async def test_multiple_session_operations_performance(
    get_valid_config, mock_user_context
):
    """
    ...
    with patch('src.cli.auth_manager.LabArchivesAPI') as mock_api_class:
        ...
        assert (
            duration < 1.0
        ), f"Session operations took {duration} seconds, expected < 1.0"
...
@pytest.mark.asyncio
async def test_authentication_methods_parameterized(
    auth_method, username, mock_user_context
):
    """
