...
class TestFolderPathFromRaw:
    ...
    def test_valid_path_parsing(
        self, raw_path: str, expected_components: Tuple[str, ...]
    ):
...
class TestFolderPathIsParentOf:
    ...
    @pytest.mark.parametrize(
        "parent_path,child_path,expected_result",
        [
    ...
    def test_is_parent_of_comprehensive(
        self, parent_path: str, child_path: str, expected_result: bool
    ):
...
class TestFolderPathValidationScenarios:
    ...
    def test_string_vs_tuple_consistency(self):
        ...
        assert string_path.is_parent_of(test_child) == tuple_path.is_parent_of(
            test_child
        )
