...
class TestSanitizeEnvVar:
    ...
    def test_sanitize_env_var_removes_invalid_characters(self):
        ...
        for input_var, expected in test_cases:
            ...
            assert (
                result == expected
            ), f"For input '{input_var}', expected '{expected}', got '{result}'"
...
class TestDeepMergeDicts:
    ...
    def test_deep_merge_dicts_handles_overlapping_keys(self):
        ...
        # Arrange
        dict1 = {
            "config": {"debug": True, "features": {"auth": True, "cache": False}},
            "version": "1.0.0",
        }
        ...
        expected = {
            "config": {
                "debug": False,
                "timeout": 60,
                "features": {"auth": True, "cache": True, "monitoring": True},
            },
            "version": "1.0.0",
    ...
    def test_deep_merge_dicts_deeply_nested(self):
        ...
        # Arrange
        dict1 = {"level1": {"level2": {"level3": {"level4": {"value": "deep1"}}}}}

        dict2 = {
            "level1": {"level2": {"level3": {"level4": {"another_value": "deep2"}}}}
        }
        ...
        expected = {
            "level1": {
                "level2": {
                    "level3": {"level4": {"value": "deep1", "another_value": "deep2"}}
                }
            }
        }
...
class TestParseIsoDatetime:
    ...
    def test_parse_iso_datetime_valid_and_invalid(self):
        ...
        valid_test_cases = [
            ...
            (
                ...
                datetime(2024, 1, 15, 10, 30, 45, 123000, tzinfo=timezone.utc),
            ),
        ...
        for iso_string, expected_datetime in valid_test_cases:
            ...
            ), f"For input '{iso_string}', expected {expected_datetime}, got {result}"
            assert isinstance(
                result, datetime
            ), f"Result should be datetime object, got {type(result)}"
    ...
    def test_parse_iso_datetime_timezone_handling(self):
        ...
        for iso_string, expected_tz in timezone_test_cases:
            ...
            # Assert
            assert (
                result.tzinfo == expected_tz
            ), f"For input '{iso_string}', expected timezone {expected_tz}, got {result.tzinfo}"
    ...
    def test_parse_iso_datetime_microsecond_precision(self):
        ...
        for iso_string, expected_microseconds in microsecond_test_cases:
            ...
            # Assert
            assert (
                result.microsecond == expected_microseconds
            ), f"For input '{iso_string}', expected {expected_microseconds} microseconds, got {result.microsecond}"
...
class TestFlattenDict:
    ...
    def test_flatten_dict_handles_nested(self):
        ...
        for key in result.keys():
            assert not isinstance(
                result[key], dict
            ), f"Value for key '{key}' should not be a dict"

    def test_flatten_dict_custom_separator(self):
        """Test flatten_dict with custom separator for compound keys."""
        # Arrange
        nested_dict = {"section": {"subsection": {"key": "value"}}}
        ...
        for separator in separators:
            ...
            assert (
                result == expected
            ), f"With separator '{separator}', expected {expected}, got {result}"
    ...
    def test_flatten_dict_deeply_nested(self):
        ...
        # Assert
        expected = {"level1.level2.level3.level4.level5.deep_value": "found"}
    ...
    def test_flatten_dict_empty_dicts(self):
        ...
        nested_with_empty = {"empty_nested": {}, "normal_key": "value"}
    ...
    def test_flatten_dict_preserves_key_order(self):
        ...
        ), f"Expected key order {expected_keys}, got {result_keys}"
    ...
    def test_flatten_dict_edge_cases(self):
        ...
        dict_with_none = {"key1": None, "nested": {"key2": None}}
        result = flatten_dict(dict_with_none)
        expected = {"key1": None, "nested.key2": None}
        assert result == expected
