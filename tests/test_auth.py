...
class StubAsyncClient:
    ...
    async def request(
        ...
        **kwargs: Any,
        ...
        call = {"method": method, "url": url, "params": params, "data": data}
        self.calls.append(call)
...
def test_ensure_uid_fetches_via_user_access_info(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def handler(
        ...
        request = httpx.Request("GET", url, params=params)
        xml_response = "<users><id>uid-123</id></users>"
        return httpx.Response(200, content=xml_response.encode(), request=request)
    ...
    call = client.calls[0]
    assert call["url"] == "https://api.labarchives.com/api/users/user_access_info"
